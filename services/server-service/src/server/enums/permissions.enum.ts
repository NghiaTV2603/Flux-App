...
export const DEFAULT_PERMISSIONS = {
  ...
  OWNER: Object.values(PERMISSION_BITS).reduce((acc, bit) => acc | bit, BigInt(0)),
  
  // Default member permissions
  MEMBER: PERMISSION_BITS[ServerPermission.VIEW_CHANNELS] |
  ...
  ADMIN: PERMISSION_BITS[ServerPermission.MANAGE_CHANNELS] |
...
export class PermissionHelper {
  static hasPermission(userPermissions: bigint, permission: ServerPermission): boolean {
    return (userPermissions & PERMISSION_BITS[permission]) !== BigInt(0);
  }

  static addPermission(currentPermissions: bigint, permission: ServerPermission): bigint {
    return currentPermissions | PERMISSION_BITS[permission];
  }

  static removePermission(currentPermissions: bigint, permission: ServerPermission): bigint {
    return currentPermissions & ~PERMISSION_BITS[permission];
  }

  static hasAnyPermission(userPermissions: bigint, permissions: ServerPermission[]): boolean {
    return permissions.some(permission => this.hasPermission(userPermissions, permission));
  }

  static hasAllPermissions(userPermissions: bigint, permissions: ServerPermission[]): boolean {
  ...
  static getPermissionList(permissions: bigint): ServerPermission[] {
    return Object.entries(PERMISSION_BITS)
      .filter(([_, bit]) => (permissions & bit) !== BigInt(0))
      .map(([permission, _]) => permission as ServerPermission);
  }
}
